...
interface CognitiveKPIProps {
    ...
    explanation?: string; // For the hover tooltip
    icon?: any;
}

export function CognitiveKPI({ label, value, change, trend, insight, explanation, icon: Icon }: CognitiveKPIProps) {
    ...
    return (
        <TooltipProvider>
            <Tooltip>
                ...
                {explanation && (
                    <TooltipContent className="bg-slate-900 border-slate-800 text-xs text-white max-w-xs p-3">
                        <div className="space-y-1">
                            <p className="font-bold text-primary italic uppercase tracking-widest text-[9px]">Análisis Profundo</p>
                            <p>{explanation}</p>
                        </div>
                    </TooltipContent>
                )}
